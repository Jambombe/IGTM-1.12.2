package net.J2P.IGTM.utils;

// Contient les nom des objets, blocs ...
public class StringNames {


    public static final String MODID = "igtm";
    public static final String VERSION = "1.0.1";
    public static final String NAME = "I Got The Money";

    // PROXY
    public static final String CLIENT_PROXY = "net.J2P.IGTM.proxy.ClientProxy";
    public static final String SERVER_PROXY = "net.J2P.IGTM.proxy.ServerProxy";

    // ITEMS
    public static final String GOLD_COIN = "gold_coin";
    public static final String SILVER_COIN = "silver_coin";
    public static final String COPPER_COIN = "copper_coin";


    // BLOCKS
    public static final String EXCHANGER = "exchanger";

    public static final String PURE_GOLD_ORE = "pure_gold_ore";
    public static final String PURE_SILVER_ORE = "pure_silver_ore";
    public static final String PURE_COPPER_ORE = "pure_copper_ore";

    // INVENTORY
    public static final String POUCH_INVENTORY = "pouch_inventory";


    // TileEntity Owner




}
